...
exports.MoogREST=function(options) {
    ...
    that.connection = that.options.connection;
    ...
    that.url = urlParser.parse(that.options.url);
    that.eventHeaders = {
    ...
    if (that.url.protocol === 'https:'){
        ...
        if (that.certFile) {
            if (!fs.existsSync(that.certFile)) {
                console.log("ERROR: Can't read :"+that.certFile);
                return;
            }
            try {
        ...
        if (that.caFile) {
            if (!fs.existsSync(that.caFile)) {
                console.log("ERROR: Can't read :"+that.caFile);
